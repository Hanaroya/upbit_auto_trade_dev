...
def sell_balanced_portfolio(total_am, curs, conn):
    ...
    if (total_profit > (abs(total_loss) + 1.5)):
        # 이익이 손실을 커버할 수 있는 경우
...
def selling_process(c_list, t_record, sma200, total_am:float, curs, conn): # 가지고 있는 코인 판매 가능 체크
    ...
    checker = 0.85
    if ubmi < 100: checker = 0.3
    if up_chk_b > checker and (str(t_record['position']).find('emergency') == -1 or str(t_record['position']).find('reach profit point') == -1): 
