...
def sell_balanced_portfolio(total_am, curs, conn):
    ...
    if (total_profit > (abs(total_loss) + 0.7)) or (total_loss < -3.95):
        # 이익이 손실을 커버할 수 있는 경우
...
def selling_process(c_list, t_record, sma200, total_am:float, curs, conn): # 가지고 있는 코인 판매 가능 체크
    ...
    if (case3_check(trade_factors=c_list) == True or (
        case4_check(trade_factors=c_list, sma200=sma200, ubmi=ubmi, ubmi_before=ubmi_before, up_chk_b=up_chk_b) == True)) and (
            t_record['hold'] == True):
        if up_chk_b > 0.05: 
