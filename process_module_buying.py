...
def case2_check(trade_factors):
    ...
    if trade_factors.iloc[-1]['signal'] < 0:
        if ((trade_factors.iloc[-1]['signal'] * 0.6) < trade_factors.iloc[-1]['macd']
            ) and ((trade_factors.iloc[-2]['signal'] * 1.03) < trade_factors.iloc[-2]['macd'] < (trade_factors.iloc[-2]['signal'] * 0.6)
        ...
        if ((trade_factors.iloc[-1]['signal'] * 1.4) < trade_factors.iloc[-1]['macd']
            ) and ((trade_factors.iloc[-2]['signal'] * 0.97) < trade_factors.iloc[-2]['macd'] < (trade_factors.iloc[-2]['signal'] * 1.4)
...
def buying_process(trade_factors, sma200, c_rank, t_record, total_am:float, curs, conn):
    ...
    if cp > t_record['record']['case1_chk'] * 1.001: t_record['record']['case1_chk'] = 0
    if cp < t_record['record']['case1_chk'] * 0.9999: t_record['record']['case1_chk'] = cp
    if cp < t_record['record']['case2_chk'] * 0.9999: t_record['record']['case2_chk'] = cp
