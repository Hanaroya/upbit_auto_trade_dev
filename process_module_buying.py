...
def case2_check(trade_factors): # MACD 크로스 상승세 확인
    if trade_factors.iloc[-1]['signal'] < 0:
        if ((trade_factors.iloc[-1]['signal'] * 0.95) < trade_factors.iloc[-1]['macd'] < (trade_factors.iloc[-1]['signal'] * 0.75)):
            return True
    elif trade_factors.iloc[-1]['signal'] > 0:
        if ((trade_factors.iloc[-1]['signal'] * 1.05) < trade_factors.iloc[-1]['macd'] < (trade_factors.iloc[-1]['signal'] * 1.20)):
            return True
    return False
...
def buying_process(trade_factors, sma200, c_rank, t_record, total_am:float, curs, conn):
    ...
    condition1 = t_record['record']['case1_chk'] > 0 and t_record['hold'] == False and (
        40 > trade_factors.iloc[-1]['rsi_K'] > trade_factors.iloc[-1]['rsi_D'] and (
            t_record['record']['case1_chk'] * 1.002) > cp > t_record['record']['case1_chk']) and macd_check(trade_factors=trade_factors)
