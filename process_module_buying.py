...
def case1_check(trade_factors): # MACD 크로스 하락세 확인
    if trade_factors.iloc[-1]['signal'] < 0:
        if ((trade_factors.iloc[-1]['macd'] * 0.55) < trade_factors.iloc[-1]['signal']):
            return True
    elif trade_factors.iloc[-1]['signal'] > 0:
        if ((trade_factors.iloc[-1]['macd'] * 1.45) < trade_factors.iloc[-1]['signal']):
            return True
    return False
...
def case2_check(trade_factors): # MACD 크로스 상승세 확인
    if trade_factors.iloc[-1]['signal'] < 0:
        if ((trade_factors.iloc[-1]['signal'] * 0.85) < trade_factors.iloc[-1]['macd'] < (trade_factors.iloc[-1]['signal'] * 0.6)
            ) and (trade_factors.iloc[-2]['macd'] < trade_factors.iloc[-1]['macd']):
            return True
    elif trade_factors.iloc[-1]['signal'] > 0:
        if ((trade_factors.iloc[-1]['signal'] * 1.15) < trade_factors.iloc[-1]['macd'] < (trade_factors.iloc[-1]['signal'] * 1.4)
            ) and (trade_factors.iloc[-2]['macd'] < trade_factors.iloc[-1]['macd']):
            return True
    return False
...
def buying_process(trade_factors, sma200, c_rank, t_record, total_am:float, curs, conn):
    ...
    elif trade_factors.iloc[-1]['rsi_K'] < 10 and trade_factors.iloc[-1]['rsi_D'] < 10 and cp < t_record['record']['case1_chk']: t_record['record']['case1_chk'] = cp
    elif cp < t_record['record']['case1_chk']: t_record['record']['case1_chk'] = cp
    ...
    ) and (trade_factors.iloc[-1]['rsi_K'] > 20 and trade_factors.iloc[-1]['rsi_D'] > 20)
    
    condition2 = t_record['record']['case2_chk'] > 0 and t_record['hold'] == False and t_record['record']['case2_chk'] < cp and (
        70 > trade_factors.iloc[-1]['rsi_K'] > 45 and 70 > trade_factors.iloc[-1]['rsi_D'] > 45) and (
